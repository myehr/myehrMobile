      ...
      if (this.$parent.selectedItemClass) {
        ...
        if (this.$parent.type === 'radio') {
          ...
          if (typeof this.value === 'string') {
            if(this.$parent.currentValue !=null) {
              this.$parent.currentValue = ''+this.$parent.currentValue+'';
  ...
  methods: {
    ...
    selectCheckbox () {
      ...
      if (!this.disabled) {
        ...
        if (index > -1) {
          this.$parent.currentValue = this.$parent.currentValue.split(',')
          let thisvalue = this.value;
          this.$parent.currentValue.splice(this.$parent.currentValue.findIndex((id) => (id ==thisvalue)),1)
        } else {
          if (!this.$parent.max || (this.$parent.max && (this.$parent.currentValue !== null && this.$parent.currentValue.length < this.$parent.max))) {
            if (this.$parent.currentValue=='' || !this.$parent.currentValue || !this.$parent.currentValue.length) {
              this.$parent.currentValue = []
            }else {
              this.$parent.currentValue = this.$parent.currentValue.split(',')
            }
