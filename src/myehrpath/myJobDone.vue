      ...
      <div class="box2">
        <cell-box style="height:60px;font-size:14px;color:#666" v-if="listData" v-for="(item,index) in listData" :link="item.path" is-link :key="index">
             <flexbox>
                <flexbox-item :span="2">
                  <!-- item.createPhoto -->
                    <div class="headerimg"><img src="../assets/grid_icon.png"/></div>
                </flexbox-item>
                <flexbox-item>
                  <!-- createAge   createPhoto -->
                    <div class="flex-demo">  
                        <span>{{ item.title }}</span>
                        <!-- <span style="float:right">{{ item.createAge }}</span> -->
                    </div>
                     <div class="flex-demo">
                       <span>{{ item.assigneeName }}</span>
                       <!-- <span style="float:right">{{ item.taskCreateDate }}</span> -->
                    </div>
                    <div class="flex-demo">
                       <!-- {{ item.taskCreateDate }} -->
                    </div>
