<template>
  <div>
    ...
    <!-- 添加多tab菜单模块 -->
     <tab>
      <tab-item @on-item-click="tabHandler(index)" v-show='datatable[0].childrens[index].childrens.length > 0' v-for="(item,index) in datatable[0].childrens" :key="index" :selected="index == selindex">{{ item.menuName }}</tab-item>
    </tab>
    <div class="swaperBox">
