<template>
  <div>
    ...
    <scroller lock-x scrollbar-y use-pullup :pullup-config="pullupConfig2" height="300px" ref="demo2" @on-pullup-loading="load2">
      <div class="box2">
        <!-- 2 -->
        <dir>234234523523</dir>
        <cell-box style="height:60px;font-size:14px;color:#666" v-for="(item,index) in listData" :link="item.path" is-link :key="index">
             <flexbox>
                <flexbox-item :span="2">
...
export default {
  ...
  menulist(m){
    this.$axios.post('/myehr/act/task/todoForMobile.action',{"order":"asc","offset":this.offset,"limit":this.limit,"paramsMap":{"userId":"1","flowCode":"7"},"requestParam":{},"filter":{"searchValue":""},"formId":"1041","isView":null})
        .then(function (response) {
            console.log(response,'菜单数据');
            if(m === 1){
                this.listData = response.data.rows;
            }else{
                this.listData.push(response.data.rows);
